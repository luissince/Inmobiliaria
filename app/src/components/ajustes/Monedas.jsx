...
class Monedas extends React.Component {
    ...
    fillTable = async (opcion, buscar) => {
        try {
            ...
            const result = await axios.get(import.meta.env.VITE_APP_END_POINT+'/api/moneda/list', {
    ...
    loadDataId = async (id) => {
        try {
            const result = await axios.get(import.meta.env.VITE_APP_END_POINT+"/api/moneda/id", {
            ...
            if (value) {
                try {
                    ...
                    if (this.state.idMoneda !== '') {
                        const result = await axios.post(import.meta.env.VITE_APP_END_POINT+'/api/moneda/update', {
                        ...
                        const result = await axios.post(import.meta.env.VITE_APP_END_POINT+'/api/moneda/add', {
                            "nombre": this.state.nombre.trim().toUpperCase(),
                            "codiso": this.state.codIso.trim().toUpperCase(),
                            "simbolo": this.state.simbolo.trim().toUpperCase(),
                            "estado": this.state.estado,
    ...
    onEventDelete(idMoneda) {
        ModalAlertDialog("Moneda", "¿Estás seguro de eliminar la moneda?", async (event) => {
            if (event) {
                try {
                    ModalAlertInfo("Moneda", "Procesando información...")
                    let result = await axios.delete(import.meta.env.VITE_APP_END_POINT+'/api/moneda', {
                        params: {
                            "idMoneda": idMoneda
                        }
                    })
