...
class MetodosPago extends React.Component {
  ...
  fillTable = async (opcion, buscar) => {
    try {
      ...
      const result = await axios.get('/api/metodopago/list', {
  ...
  loadDataId = async (id) => {
    try {
      if (id !== "") {
        const result = await axios.get("/api/metodoPago/id", {
        ...
        const result = await axios.get("/api/metodoPago/codigo", {
          signal: this.abortControllerModal.signal,
        });
    ...
    if (this.state.nombre === "") {
      ...
      try {
        ...
        if (this.state.idMetodo !== "") {
          const result = await axios.post('/api/metodoPago/edit', {
          ...
          const result = await axios.post('/api/metodoPago/add', {
            "nombre": this.state.nombre,
            "codigo": this.state.codigo,
            "estado": this.state.estado,
            "idUsuario": this.state.idUsuario,
    ...
    ModalAlertDialog("metodoPago", "¿Estás seguro de eliminar el metodo de pago?", async (event) => {
      if (event) {
        try {
          ...
          let result = await axios.delete('/api/metodoPago', {
            params: {
