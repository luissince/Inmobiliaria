...
class Comprobantes extends React.Component {
    ...
    fillTable = async (opcion, buscar) => {
        try {
            ...
            const result = await axios.get(import.meta.env.VITE_APP_END_POINT+'/api/comprobante/list', {
    ...
    loadDataId = async (id) => {
        try {
            const result = await axios.get(import.meta.env.VITE_APP_END_POINT+"/api/comprobante/id", {
    ...
    async onEventGuardar() {
        ...
        try {
            ...
            if (this.state.idComprobante !== "") {
                const result = await axios.post(import.meta.env.VITE_APP_END_POINT+'/api/comprobante/edit', {
                ...
                const result = await axios.post(import.meta.env.VITE_APP_END_POINT+'/api/comprobante/add', {
                    "tipo": this.state.tipo,
                    "nombre": this.state.nombre.trim().toUpperCase(),
                    "serie": this.state.serie.trim().toUpperCase(),
                    "numeracion": this.state.numeracion,
    ...
    onEventDelete(idComprobante) {
        ModalAlertDialog("Comprobante", "¿Estás seguro de eliminar el comprobante?", async (event) => {
            if (event) {
                try {
                    ModalAlertInfo("Comprobante", "Procesando información...")
                    let result = await axios.delete(import.meta.env.VITE_APP_END_POINT+'/api/comprobante', {
                        params: {
