...
class Bancos extends React.Component {
  ...
  loadDataId = async (id) => {
    try {
      ...
      const banco = await axios.get(import.meta.env.VITE_APP_END_POINT+"/api/banco/id", {
        signal: this.abortControllerModal.signal,
        params: {
          idBanco: id,
        },
    ...
    if (this.state.nombre === "") {
      ...
      try {
        ...
        if (this.state.idBanco !== "") {
          const result = await axios.put(import.meta.env.VITE_APP_END_POINT+"/api/banco/", {
          ...
          const result = await axios.post(import.meta.env.VITE_APP_END_POINT+"/api/banco/", {
            nombre: this.state.nombre.trim().toUpperCase(),
            tipoCuenta: this.state.tipoCuenta,
            idMoneda: this.state.idMoneda.trim().toUpperCase(),
            numCuenta: this.state.numCuenta.trim().toUpperCase(),
    ...
    ModalAlertDialog(
      ...
      async (event) => {
        if (event) {
          try {
            ModalAlertInfo("Banco", "Procesando información...");
            let result = await axios.delete(import.meta.env.VITE_APP_END_POINT+"/api/banco", {
              params: {
                idBanco: idBanco,
              },
            });
