...
class ClienteProceso extends React.Component {
    ...
    loadData = async () => {
        try {
            const documento = await axios.get(import.meta.env.VITE_APP_END_POINT+"/api/tipodocumento/listcombo", {
    ...
    loadDataId = async (id) => {
        try {
            ...
            const result = await axios.get(import.meta.env.VITE_APP_END_POINT+"/api/cliente/id", {
                signal: this.abortControllerTable.signal,
                params: {
                    "idCliente": id
                }
    ...
    async onEventGuardar() {
        ...
        try {
            ...
            if (this.state.idCliente !== '') {
                const cliente = await axios.post(import.meta.env.VITE_APP_END_POINT+'/api/cliente/update', {
                ...
                const cliente = await axios.post(import.meta.env.VITE_APP_END_POINT+'/api/cliente/add', {
                    "idTipoDocumento": this.state.idTipoDocumento,
                    "documento": this.state.documento.toString().trim().toUpperCase(),
                    "informacion": this.state.informacion.trim().toUpperCase(),
                    "telefono": this.state.telefono.toString().trim().toUpperCase(),
    ...
    handleFilter = async (event) => {
        ...
        try {
            ...
            let result = await axios.get(import.meta.env.VITE_APP_END_POINT+"/api/ubigeo/", {
                params: {
                    filtrar: searchWord,
                },
            });
