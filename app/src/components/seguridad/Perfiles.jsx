...
class Perfiles extends React.Component {
    ...
    fillTable = async (opcion, buscar) => {
        try {
            ...
            const result = await axios.get(import.meta.env.VITE_APP_END_POINT+'/api/perfil/list', {
    ...
    loadDataId = async (id) => {
        try {
            const perfil = await axios.get(import.meta.env.VITE_APP_END_POINT+"/api/perfil/id", {
            ...
            if (event) {
                try {
                    ...
                    if (this.state.idPerfil !== '') {
                        let result = await axios.post(import.meta.env.VITE_APP_END_POINT+'/api/perfil/update', {
                        ...
                        let result = await axios.post(import.meta.env.VITE_APP_END_POINT+'/api/perfil/add', {
                            "descripcion": this.state.descripcion.trim().toUpperCase(),
                            "idSede": "SD0001",
                            "idUsuario": this.state.idUsuario,
                        }, {
    ...
    onEventDelete(idPerfil) {
        ModalAlertDialog("Perfil", "¿Estás seguro de eliminar el perfil?", async (event) => {
            if (event) {
                try {
                    ModalAlertInfo("Perfil", "Procesando información...")
                    let result = await axios.delete(import.meta.env.VITE_APP_END_POINT+'/api/perfil', {
                        headers: {
                            Authorization: "Bearer " + this.state.token
                        },
                        params: {
